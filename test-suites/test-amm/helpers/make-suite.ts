...
export async function initializeMakeSuite() {
  ...
  const aDaiAddress = allTokens.find((uToken) => uToken.symbol === 'aAmmDAI')?.tokenAddress;

  const aWEthAddress = allTokens.find((uToken) => uToken.symbol === 'aAmmWETH')?.tokenAddress;

  const reservesTokens = await testEnv.helpersContract.getAllReservesTokens();

  const daiAddress = reservesTokens.find((token) => token.symbol === 'DAI')?.tokenAddress;
  const usdcAddress = reservesTokens.find((token) => token.symbol === 'USDC')?.tokenAddress;
  const umeeAddress = reservesTokens.find((token) => token.symbol === 'UniUMEEWETH')?.tokenAddress;
