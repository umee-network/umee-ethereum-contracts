...
makeSuite('Upgradeability', (testEnv: TestEnv) => {
  ...
  before('deploying instances', async () => {
    ...
    const uTokenInstance = await deployMockUToken([
      ...
      ZERO_ADDRESS,
      ZERO_ADDRESS,
      'Umee Interest bearing DAI updated',
      'aDAI',
      '0x10'
    ]);
    ...
    const stableDebtTokenInstance = await deployMockStableDebtToken([
      ...
      dai.address,
      ZERO_ADDRESS,
      'Umee stable debt bearing DAI updated',
      'stableDebtDAI',
      '0x10'
    ]);
    ...
    const variableDebtTokenInstance = await deployMockVariableDebtToken([
      ...
      dai.address,
      ZERO_ADDRESS,
      'Umee variable debt bearing DAI updated',
      'variableDebtDAI',
      '0x10'
    ]);
  ...
  it('Upgrades the DAI Atoken implementation ', async () => {
    ...
    expect(tokenName).to.be.eq('Umee Interest bearing DAI updated', 'Invalid token name');
  });
  ...
  it('Upgrades the DAI stable debt token implementation ', async () => {
    ...
    expect(tokenName).to.be.eq('Umee stable debt bearing DAI updated', 'Invalid token name');
  });
  ...
  it('Upgrades the DAI variable debt token implementation ', async () => {
    ...
    expect(tokenName).to.be.eq('Umee variable debt bearing DAI updated', 'Invalid token name');
  });
});
